                            ...
                            return (
                                <tr key={r.id ?? rIdx} className={`hover:bg-slate-50/80 transition-colors group border-b border-slate-100 last:border-0 ${rIdx % 2 === 0 ? 'bg-white' : 'bg-slate-50/30'} ${r.id && selectedIds.has(r.id) ? 'bg-indigo-50/50' : ''}`}>
                                    ...
                                    <td className="px-3 py-2 text-center text-[12px]">
                                        ...
                                        <div className="text-[10px] text-slate-500 font-semibold mt-0.5 opacity-60">({r.occupantName || 'स्वतः'})</div>
                                    </td>
                                    <td className="px-3 py-2 text-center text-[12px]">
                                        {r.sections?.filter(s => s.propertyType && s.propertyType !== 'निवडा').map((s, si) => (
                                            <div key={si} className="mb-1.5 border-b border-slate-100 last:border-0 pb-1.5 leading-tight">
                                                <div className="text-[11px] font-black text-slate-700 uppercase">{s.propertyType}</div>
                                                <div className="flex items-center justify-center gap-1 mt-0.5">
                                                    <span className="font-bold text-slate-900 text-[12px]">{MN(s.areaSqFt)}</span>
                                                    <span className="text-[10px] text-slate-400 font-bold uppercase">चौ.फु</span>
                                                </div>
                                            </div>
                                        )) || '-'}
                                    </td>
                                    <td className="px-3 py-2 text-start text-[12px]">
                                        <div className="text-[12px] space-y-0.5 text-slate-500 font-bold tracking-tight leading-none inline-block">
                                    ...
                                    <td className="px-3 py-2 text-center min-w-[110px]">
                                        {r.receiptNo ? (
                                            ...
                                            <div className="flex flex-col items-center gap-0.5">
                                                ...
                                                <div className="text-[10px] font-black text-emerald-700">₹{MN(paid.toFixed(0))}</div>
